...
**[Модификаторы доступа](https://learn.microsoft.com/ru-ru/dotnet/csharp/language-reference/keywords/access-modifiers) ([Access Modifiers](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/access-modifiers))**
...
Вы правы, в C# действительно можно объявлять член интерфейса с уровнем доступа `private`, но только если он имеет реализацию по умолчанию. Это полезно для обеспечения инкапсуляции и ограничения доступа к методам, которые предназначены только для внутреннего использования в интерфейсе. Обновленная таблица с учетом этого замечания:
