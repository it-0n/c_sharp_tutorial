...
Обратите внимание, как определено в этом файле пространство имен. Всего одной строкой. То есть без открывающей и
закрывающей фигурных скобок. Этот синтаксис появился в .NET 6 SDK. И может использоваться если в файле определено только
одно пространство имен. Да в одном файле может быть определено несколько пространств имен, но так обычно не делается.

И посмотрите как это сделано ниже. Можно использовать оба варианта. Но если в файле только одно пространство имен, то
удобнее использовать новый вариант.
...
Падван, для тебя будет практическое задание.

Создай консольный проект ex0014_pt01 в каталоге episode02 из стандартного шаблона console. Подключи его к решению `episode02.sln`.
И перенеси в Program.cs последний вариант кода из проекта `ex0014_global_using`. Естественно тебе его придётся немного 
изменить чтобы он заработал. Дополнительное условие: файл проекта не должен меняться. То есть должен выглядеть вот так:
...
```
Подключать пространства имен, можете как локально так и глобально, но не в файле проекта. Так же можете все инструкции
программы определить в одном файле `Program.cs` или же разбить на два файла, как мы это делали в последнем примере. Или даже
на три, если решите использовать глобальные инструкции `using`. Тут на ваше усмотрение.

Подсказка команды для создания проекта:

![Создание консольного приложения console](NameSpaces42.png){ border-effect="line"  thumbnail="true" width="700" }

Всё остальное уже сам, падаван!

Какой вывод должен быть у программы:

![Создание консольного приложения console](NameSpaces43.png){ border-effect="line"  thumbnail="true" width="700" }