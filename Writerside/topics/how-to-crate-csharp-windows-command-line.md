...
проторенными тропами. Но мы не ищем лёгких путей.! Поэтому даже если что-то и будет не понятно, или даже все не будет понятно,
...
консоль сообщение `Hello, World!`. Каждая инструкция (statement) должна заканчиваться точкой с запятой **;** которая означает
конец инструкции.
