...
1. **Стандартные форматы:** простые коды вроде `"d"` (только дата) или `"t"` (только время).
2. **Кастомные форматы:** полный контроль над представлением (например, `"dd.MM.yyyy HH:mm"`).
