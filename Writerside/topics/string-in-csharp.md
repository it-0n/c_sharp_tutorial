...
Работа со строками в .NET — это как магия: из обычных букв можно создавать целые числа, даты, символы и даже включать/выключать 
свет (если понимать строки как булевые значения). Здесь мы поговорим о том, как в .NET превращать строки в символы (**Char**), 
флаги **Boolean** и даже элементы перечислений (**Enum**). Приступим!
