...
## Обрезка и удаление символов из строки
...
В предыдущем разделе мы уже чуток познакомились с методом  `String.Replace`, но использовали его для удаления символов 
в строке, но он может делать не только это.

Методы [String.Replace](https://learn.microsoft.com/ru-ru/dotnet/api/system.string.replace) и 
[String.ReplaceLineEndings](https://learn.microsoft.com/ru-ru/dotnet/api/system.string.replacelineendings?) 
позволяют изменять содержимое строк, заменяя символы, подстроки или символы перевода строки. 
Сейчас мы разберём каждый метод, его перегрузки (варианты) на понятных примерах.

### Метод `String.Replace` {id="string-replace_1"}

#### Что делает?

Метод `String.Replace` заменяет один символ или подстроку другим символом или подстрокой. Это полезно, когда вы хотите изменить определённые части текста, например, заменить все пробелы на подчёркивания или исправить опечатки.

#### Перегрузки `String.Replace`

| Перегрузка                                      | Описание                                                                                       |  
|------------------------------------------------|-----------------------------------------------------------------------------------------------|  
| `Replace(Char, Char)`                          | Заменяет указанный символ на другой.                                                          |  
| `Replace(String, String)`                      | Заменяет указанную подстроку на другую.                                                       |  
| `Replace(String, String, StringComparison)`    | Заменяет указанную подстроку на другую, учитывая правила сравнения строк (например, регистр). |  
| `Replace(String, String, Boolean, CultureInfo)`| Заменяет указанную подстроку на другую, учитывая региональные настройки.                      |  

### Подробное описание с примерами

#### 1. `Replace(Char, Char)`

Заменяет один символ другим.

**Пример:**

```c#
string text = "apple";
string result = text.Replace('a', 'o'); // Заменяем 'a' на 'o'
Console.WriteLine(result); // Вывод: "opple"
```  
...
string result = text.Replace("love", "like"); // Заменяем "love" на "like"
Console.WriteLine(result); // Вывод: "I like programming."
```  

#### 3. `Replace(String, String, StringComparison)`

Заменяет одну подстроку на другую, учитывая правила сравнения строк. Например, можно игнорировать регистр.

**Пример:**

```c#
string text = "Hello world!";
string result = text.Replace("hello", "Hi", StringComparison.OrdinalIgnoreCase); // Игнорируем регистр
Console.WriteLine(result); // Вывод: "Hi world!"
```  

#### 4. `Replace(String, String, Boolean, CultureInfo)`

Заменяет одну подстроку на другую с учётом региональных настроек (например, особенности языков).

Параметр `Boolean` опеределяет игнорировать регистр (ture) при сравнении, либо учитывать (false).
...
string text = "Straße"; // Немецкое слово "улица"
string result = text.Replace("ß", "ss", false, new CultureInfo("de-DE")); // Немецкий язык
Console.WriteLine(result); // Вывод: "Strasse"
```  

### Метод `String.ReplaceLineEndings` {id="string-replacelineendings_1"}

#### Что делает?

Метод `String.ReplaceLineEndings` заменяет символы перевода строки в строке на указанный символ или стандартный символ перевода строки `Environment.NewLine`. Это полезно для работы с текстами, созданными на разных операционных системах, где символы перевода строки отличаются.

#### Перегрузки `String.ReplaceLineEndings`

| Перегрузка                 | Описание                                                                                  |  
|----------------------------|------------------------------------------------------------------------------------------|  
| `ReplaceLineEndings()`     | Заменяет символы перевода строки на системный символ `Environment.NewLine`.              |  
| `ReplaceLineEndings(String)` | Заменяет символы перевода строки на указанный символ или строку.                         |  

### Подробное описание с примерами

#### 1. `ReplaceLineEndings()`

Заменяет все символы перевода строки (например, `\n`, `\r\n`, `\r`) на `Environment.NewLine`.

**Пример:**

```c#
string text = "Hello\nWorld!\r\nHow are you?\r";
string result = text.ReplaceLineEndings(); // Заменяем на символ перевода строки ОС
Console.WriteLine(result);
```  

#### 2. `ReplaceLineEndings(String)`

Заменяет все символы перевода строки на указанный символ или строку.

**Пример:**

```c#
string text = "Hello\nWorld!\r\nHow are you?\r";
string result = text.ReplaceLineEndings("---"); // Заменяем на "---"
Console.WriteLine(result);
...
Создай консольное приложение `ex0067_string_replace` при помощи шаблона `tinyconsole` в папке `episode02` и добавь его в
файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
## Интернированные строки и методы

Строки в C# — это неизменяемые объекты, которые занимают место в памяти. Чтобы оптимизировать использование памяти и 
повысить производительность, .NET применяет механизм интернирования строк. Сейчас мы разберём, что такое 
интернированные строки, как работают методы [String.Intern](https://learn.microsoft.com/ru-ru/dotnet/api/system.string.intern) 
и [String.IsInterned](https://learn.microsoft.com/ru-ru/dotnet/api/system.string.isinterned), и рассмотрим примеры кода.

### Что такое интернированная строка?

**Интернированная строка** — это строка, которая хранится в специальной области памяти, называемой **пулом строк** (string intern pool). Если строка интернирована, то её экземпляр не создаётся заново в памяти, если точно такая же строка уже есть в этом пуле. Вместо этого используется ссылка на существующую строку.

#### Пример:
Если две строки `"Hello"` и `"Hello"` используются в программе, они могут занимать одно и то же место в памяти благодаря интернированию.

### Метод `String.Intern`

#### Что делает?

Метод `String.Intern` добавляет строку в пул строк, если её там ещё нет. Если строка уже есть в пуле, то метод возвращает ссылку на интернированную строку.

#### Когда применять?

Используйте `String.Intern`, если вы работаете с большим количеством одинаковых строк, чтобы уменьшить использование памяти. Это особенно полезно для строк, которые часто повторяются в программе.
...
class Program
{
    static void Main()
    {
        string str1 = "Hello"; // Автоматически интернированная строка
        string str2 = new string("Hello".ToCharArray()); // Не интернированная строка

        Console.WriteLine(Object.ReferenceEquals(str1, str2)); // False (разные объекты)

        string internedStr = String.Intern(str2); // Интернируем str2
        Console.WriteLine(Object.ReferenceEquals(str1, internedStr)); // True (теперь оба объекта ссылаются на одну строку в памяти)
    }
}
```
...
В этом примере `str1` и `str2` изначально занимают разные места в памяти. После вызова `String.Intern` строка `str2` начинает ссылаться на интернированную строку.

### Метод `String.IsInterned`

#### Что делает?

Метод `String.IsInterned` проверяет, находится ли строка в пуле строк. Если строка интернирована, метод возвращает ссылку на неё. Если нет — возвращает `null`.
...
class Program
{
    static void Main()
    {
        string str1 = "Hello"; // Интернированная строка
        string str2 = new string("Hello".ToCharArray()); // Не интернированная строка

        Console.WriteLine(String.IsInterned(str1) != null ? "str1 интернирована" : "str1 не интернирована"); // str1 интернирована
        Console.WriteLine(String.IsInterned(str2) != null ? "str2 интернирована" : "str2 не интернирована"); // str2 не интернирована

        String.Intern(str2); // Интернируем str2
        Console.WriteLine(String.IsInterned(str2) != null ? "str2 интернирована" : "str2 не интернирована"); // str2 интернирована
    }
}
```
...
```
str1 интернирована  
str2 не интернирована  
str2 интернирована  
```

Здесь мы видим, что `str1` изначально интернирована, а `str2` нет. После вызова `String.Intern`, строка `str2` становится интернированной.

Время практики!

Создай консольное приложение `ex0068_string_intern` при помощи шаблона `tinyconsole` в папке `episode02` и добавь его в
файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
- **Интернированные строки** позволяют экономить память, используя пул строк для хранения уникальных строковых значений.
- Метод `String.Intern` добавляет строку в пул строк и возвращает ссылку на интернированную строку.
- Метод `String.IsInterned` проверяет, интернирована ли строка, и возвращает ссылку на неё, если она есть в пуле.

Эти методы полезны для оптимизации приложений, работающих с большим количеством повторяющихся строк.

## Анализ и обработка чисел в строках

Работа с числами — важная часть программирования. Мы часто преобразуем [строки в числа](https://learn.microsoft.com/ru-ru/dotnet/standard/base-types/parsing-numeric), 
чтобы сделать вычисления, сохранить данные в базе или просто вывести их красиво на экран. В этом разделе мы разберём, 
как в .NET справляются с такими задачами, используя методы `Parse` и `TryParse`. А ещё поговорим о [стандартных](https://learn.microsoft.com/ru-ru/dotnet/standard/base-types/standard-numeric-format-strings) и 
[настраиваемых](https://learn.microsoft.com/ru-ru/dotnet/standard/base-types/custom-numeric-format-strings) форматах чисел, поставщиках форматов, значениях `NumberStyles` и даже о цифрах в Юникоде!

### Зачем нам анализ числовых строк?

Иногда в вашем приложении строка `"42"` — это не просто строка, а число, с которым хочется посчитать, например, 
возраст котика в кошачьих годах. Вот тут-то и приходят на помощь методы `Parse` и `TryParse`.

Эти методы помогают взять строку, которая выглядит как число, и превратить её в настоящий числовой тип (`int`, `float`, `double`, и т.д.). Но это ещё не всё! Благодаря строкам форматов мы можем красиво выводить числа, например:
- **Стандартные форматы** — позволяют быстро форматировать числа в привычные виды, например, в валюту (`C`) или в проценты (`P`).
- **Настраиваемые форматы** — дают больше гибкости, чтобы создавать свои уникальные шаблоны отображения.

### Синтаксический анализ и поставщики форматов

Когда вы передаёте строку в `Parse` или `TryParse`, .NET должен как-то понять, как её преобразовать в число. 
Например, в США десятичная часть числа отделяется точкой (`3.14`), а в Германии — запятой (`3,14`). Здесь на сцену 
выходят **поставщики форматов** (они же `IFormatProvider`).

#### Поставщики форматов — кто они такие?

Это как официанты в ресторане, которые подают вам числа в "правильной упаковке". Самый популярный 
"официант" — это `CultureInfo`. Он отвечает за то, как числа и даты выглядят в зависимости от культуры. Например:
- В культуре `"en-US"` (США) мы пишем `1,234.56`.
- В культуре `"de-DE"` (Германия) это будет `1.234,56`.

#### Пример кода: Синтаксический анализ с форматами
...
class Program
{
    static void Main()
    {
        ...
        Console.WriteLine($"US формат: {usNumber}"); // 1234.56
        ...
        try
        {
            ...
            Console.WriteLine($"German формат: {germanNumber}");
        }
        catch (FormatException)
        {
            Console.WriteLine("Упс! Германия не понимает такой формат числа!");
        }
    }
}
```
...
```
US формат: 1234.56  
Упс! Германия не понимает такой формат числа!  
```

.NET умеет подстраиваться под культурные особенности, но будьте внимательны, когда работаете с числами из разных регионов.

### Синтаксический анализ и значения NumberStyles

Иногда строка может содержать символы, которые на первый взгляд мешают понять, что это число: пробелы, знаки валюты, процентные знаки. Здесь на помощь приходят **значения `NumberStyles`**. Это что-то вроде "инструкций" для `Parse`, которые говорят, какие символы можно игнорировать, а какие учитывать.

#### Пример: Числа с пробелами и валютой
...
class Program
{
    static void Main()
    {
        ...
        // Анализ с учетом валюты и пробелов
        double parsedNumber = double.Parse(number, NumberStyles.Currency, CultureInfo.InvariantCulture);
        Console.WriteLine($"Число: {parsedNumber}"); // 1234.56
    }
}
```
...
В Юникоде есть символы для цифр в разных языках (например, арабский, деванагари). Если ваша программа должна работать с такими числами, .NET это тоже поддерживает.

#### Пример: Числа в разных языках
...
class Program
{
    static void Main()
    {
        string arabicNumber = "١٢٣٤"; // Это "1234" на арабском

        int parsedNumber = int.Parse(arabicNumber, NumberStyles.None, CultureInfo.InvariantCulture);
        Console.WriteLine($"Арабские цифры: {parsedNumber}"); // 1234
    }
}
```

.NET автоматически распознаёт цифры в разных языках, но важно использовать правильные параметры.

И снова практики!

Создай консольное приложение `ex0069_string_numbers` при помощи шаблона `tinyconsole` в папке `episode02` и добавь его в
файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
```
=== Синтаксический анализ с форматами ===
US формат: 1234.56
Упс! Германия не понимает такой формат числа!
...
=== Числа в Юникоде ===
Арабские цифры: 1234
Деванагари цифры: 1234
```

### Как работает программа?

1. **Метод `NormalizeUnicodeDigits`:**
   - Проходит по каждому символу строки и проверяет, является ли символ цифрой (`UnicodeCategory.DecimalDigitNumber`).
   - Преобразует цифру в Unicode в её эквивалент в диапазоне `'0-9'` с помощью `CharUnicodeInfo.GetDigitValue`.

2. **Использование нормализации перед разбором:**
   - Перед вызовом `int.Parse` строки с цифрами в Unicode преобразуются в стандартные цифры.

### Итоги

- `Parse` и `TryParse` — мощные инструменты для анализа чисел.
- Используйте `IFormatProvider`, чтобы учитывать особенности регионов.
- `NumberStyles` помогают обрабатывать сложные форматы строк.
- Юникод поддерживает цифры в разных языках, что делает приложения удобными для пользователей со всего мира.

Анализ числовых строк — это весело и полезно! 🎉

## Анализ и обработка даты и времени в строках

### Как приручить дату и время?
Каждый из нас видел странные строки вроде `"12.03.2025"` или `"March 12, 2025, 3:45 PM"`. [Компьютеру нужно понимать](https://learn.microsoft.com/ru-ru/dotnet/standard/base-types/parsing-datetime), 
как читать такие строки, чтобы преобразовать их в **объекты DateTime**. Зачем? Чтобы работать с датами и временем: 
запоминать встречи, считать, сколько дней осталось до каникул, или просто не опоздать на обед.

Но вот в чём фокус: в разных странах пишут даты по-разному. Например:
- США любят писать **месяц-день-год**: `03/12/2025`.
- Европа думает, что **день-месяц-год** — это логично: `12/03/2025`.
- Время? Кто-то говорит "PM", а кто-то предпочитает 24-часовой формат.

.NET умеет всё это разруливать, но нужно немного помочь: указать, как именно дата или время записаны, и в каком формате. А ещё есть временные зоны и "UTC", которые добавляют в эту кашу свой колорит.

#### Простой пример: как прочитать дату
Посмотрим, как преобразовать строку `"12/03/2025"` в дату:
```c#
using System;
using System.Globalization;
...
class Program
{
    static void Main()
    {
        string dateText = "12/03/2025";
        
        // Используем культуру для формата США
        var usCulture = new CultureInfo("en-US");
        DateTime date = DateTime.Parse(dateText, usCulture);
        
        Console.WriteLine($"Дата: {date}"); // Вывод: 3 декабря 2025 года
    }
}
```
👉 **Объяснение:** строка `"12/03/2025"` интерпретируется как декабрь в США (месяц-день-год).

### Метод **ParseExact**: читаем даты "под микроскопом" {id="parseexact_1"}
Бывают ситуации, когда нужно точно указать формат строки. Например, если ваша строка `"2025-12-03 15:30:45"` строго следует шаблону **"год-месяц-день часы:минуты:секунды"**, лучше использовать **ParseExact**.

#### Как работает ParseExact? {id="parseexact_2"}
- Вы указываете строку даты.
- Прописываете точный шаблон формата (например, `"yyyy-MM-dd HH:mm:ss"`).
- Метод выдаёт вам объект DateTime.

#### Немного о DateTime
**DateTime** — это супергерой среди структур в .NET, который знает всё о дате и времени:
- Какой сейчас день, месяц, год.
- Точное время.
- Разницу между локальным временем и UTC.

Когда вы преобразуете строку в DateTime, это как оживить дату: она становится чем-то, с чем можно работать (добавлять дни, сравнивать, форматировать обратно в строку и т.д.).
...
class Program
{
    static void Main()
    {
        string dateText = "2025-12-03 15:30:45";
        string format = "yyyy-MM-dd HH:mm:ss";

        DateTime parsedDate = DateTime.ParseExact(dateText, format, CultureInfo.InvariantCulture);
        
        Console.WriteLine($"Дата и время: {parsedDate}");
    }
}
```
👉 **Объяснение:**
1. Формат **"yyyy-MM-dd HH:mm:ss"** говорит:
   - `yyyy` — год в 4 цифрах,
   - `MM` — месяц в 2 цифрах,
   - `dd` — день,
   - `HH:mm:ss` — часы, минуты и секунды в 24-часовом формате.
2. `CultureInfo.InvariantCulture` означает: никаких национальных особенностей.


### Стандартные и кастомные форматы: игра с шаблонами
.NET поддерживает два типа форматов для работы с датами и временем:
1. **[Стандартные форматы](https://learn.microsoft.com/ru-ru/dotnet/standard/base-types/standard-date-and-time-format-strings):** простые коды вроде `"d"` (только дата) или `"t"` (только время).
2. **[Кастомные форматы](https://learn.microsoft.com/ru-ru/dotnet/standard/base-types/custom-date-and-time-format-strings):** полный контроль над представлением (например, `"dd.MM.yyyy HH:mm"`).

#### Пример:
```c#
string format1 = DateTime.Now.ToString("F", new CultureInfo("ru-RU")); // Полная дата на русском
string format2 = DateTime.Now.ToString("yyyy/MM/dd");                 // Год/Месяц/День
Console.WriteLine($"Формат 1: {format1}");
Console.WriteLine($"Формат 2: {format2}");
```

Попрактикуемся, падаван!

Создай консольное приложение `ex0070_string_datetime` при помощи шаблона `tinyconsole` в папке `episode02` и добавь его в
файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
#### Вывод программы:
```
Дата (США): 03.12.2025 0:00:00
Точная дата: 03.12.2025 15:30:45
Форматированная дата: 03.12.2025 15:30
Дата на французском: mercredi 3 decembre 2025 15:30:45
Текущее время в UTC: 14.01.2025 19:22:48
```

### Вывод
- Мы научились превращать строки в даты, используя **Parse** и **ParseExact**.
- Знаем, что **DateTime** — это мощный инструмент для работы с датами и временем.
- Освоили форматы и научились играть с культурами.

Теперь вы вооружены всем необходимым, чтобы победить в битве с датами и временем в .NET. Удачи, падаван! 😊

## Анализ и обработка других типов в строках

Когда текст превращается в что-то большее!

Работа со строками в .NET — это как магия: из обычных букв можно создавать целые числа, даты, символы и даже включать/выключать свет (если понимать строки как булевые значения). Здесь мы поговорим о том, как в .NET превращать строки в символы (**Char**), флаги **Boolean** и даже элементы перечислений (**Enum**). Приступим!

### 🅰️ **Char**: когда важна каждая буква

**Char** (символ) — это одна буква, цифра или знак из строки. Если строка — это слово, то **Char** — это отдельная буква в этом слове. В .NET вы можете анализировать строки, чтобы выделить из них один символ.

#### Как это работает?
.NET предоставляет метод `Char.Parse`, который берёт строку, состоящую из **одного символа**, и превращает её в тип **Char**. Если строка состоит из двух или более символов, .NET выдаст ошибку: "Эй, я хотел только одну букву!"
...
class Program
{
    ...
    {
        string charString = "A";
        char result = Char.Parse(charString);
        Console.WriteLine($"Символ: {result}"); // Вывод: Символ: A
        
        // А если строка длинная?
        try
        {
            string longString = "Hello";
            char longResult = Char.Parse(longString); // Ошибка!
        }
        catch (FormatException)
        {
            Console.WriteLine("Ошибка: строка должна содержать только один символ!");
        }
    }
}
```
...
**Boolean** (булевый тип) — это всего два значения: **true** (истина) или **false** (ложь). Превращение строк в булевое значение бывает полезно, когда нужно понять, включён ли режим "Ночной страж" или выключен.

#### Как это работает?
.NET использует метод `Boolean.Parse`, который превращает строки `"true"` или `"false"` в булевые значения. **Важно:** строка должна быть точной (без ошибок в регистре, например "True" или "False").
...
class Program
{
    static void Main()
    {
        ...
        bool isTrue = Boolean.Parse(trueString);
        bool isFalse = Boolean.Parse(falseString);

        Console.WriteLine($"Значение 1: {isTrue}"); // Вывод: true
        Console.WriteLine($"Значение 2: {isFalse}"); // Вывод: false

        // А если в строке что-то странное?
        try
        {
            string invalidString = "maybe";
            bool result = Boolean.Parse(invalidString); // Ошибка!
        }
        catch (FormatException)
        {
            Console.WriteLine("Ошибка: строка должна быть 'true' или 'false'!");
        }
    }
}
```

### 🎨 **Enum**: превращаем текст в магию перечислений

Перечисления (**Enum**) — это способ придать вашим данным смысл. Представьте себе кофейный автомат, где есть кнопки: **Espresso**, **Latte**, **Cappuccino**. Вместо того чтобы работать с числами (1, 2, 3), вы используете слова, которые понятны каждому.

В .NET можно анализировать строки, чтобы сопоставить их значения с элементами перечислений. Это делает код понятнее и позволяет избежать ошибок.

#### Как это работает?
Метод `Enum.Parse` берёт строку, сопоставляет её с именем в перечислении и возвращает соответствующий элемент. Если строка не соответствует ни одному элементу, .NET выдаёт ошибку.

#### Пример: выбираем суперсилу
Представим, что у нас есть перечисление супергероев с их способностями.
```c#
using System;
...
class Program
{
    ...
    static void Main()
    {
        ...
        // Преобразуем строку в элемент перечисления
        Superpower power = (Superpower)Enum.Parse(typeof(Superpower), input);
        Console.WriteLine($"Выбранная суперсила: {power}"); // Вывод: Выбранная суперсила: Flight

        // А если строка неверная?
        try
        {
            string invalidInput = "WaterBreathing";
            Superpower invalidPower = (Superpower)Enum.Parse(typeof(Superpower), invalidInput);
        }
        catch (ArgumentException)
        {
            Console.WriteLine("Ошибка: такой суперсилы нет!");
        }
    }
}
```

### Вывод: три типа строк, три уровня магии
- **Char** позволяет работать с отдельными символами. Помните: строка должна содержать только один символ!
- **Boolean** помогает преобразовывать строки `"true"` или `"false"` в булевы значения. Убедитесь, что регистр соблюдён!
- **Enum** добавляет осмысленность, превращая строки в элементы перечислений. Это идеальный способ работать с предопределёнными списками (например, суперспособностями, уровнями сложности игры и т.п.).

Время магии, падаван!

Создай консольное приложение `ex0071_string_other` при помощи шаблона `tinyconsole` в папке `episode02` и добавь его в
файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
```
Буква: H
Ночной режим включён: True
Вы выбрали: Latte
```

Теперь вы знаете, как превращать строки в полезные данные! Берите эту магию и улучшайте свои проекты! 😊