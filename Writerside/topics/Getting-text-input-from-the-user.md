...
Console.WriteLine($"Возраст: {age} лет, рост: {height} м, зарплата: {salary} рублей.");
```
...
И на заметку, видите метод `TryParse` у `int` и `double`? Вспоминайте, C# до мозга костей объектно-ориентированный язык.
