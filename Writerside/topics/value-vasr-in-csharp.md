...
## Порядок операций
В C# порядок выполнения операций определяется стандартными правилами, известными как **PEMDAS**:
- **P (Parentheses)**: Сначала выполняются выражения в скобках.
- **E (Exponents)**: В C# возведение в степень выполняется с помощью `Math.Pow()`, и эта операция имеет высокий приоритет.
- **MD (Multiplication and Division)**: Умножение и деление выполняются слева направо.
- **AS (Addition and Subtraction)**: Сложение и вычитание также выполняются слева направо.

Закрепим на практике.

Создай консольное приложение `ex0040_oreder_ops` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
```
Результат 1: 3 + 4 * 5 = 23
Результат 2: (3 + 4) * 5 = 35
Результат 3: 20 / 4 % 3 = 2
Результат 4: 2^3 + 5 = 13
Результат 5: (2 + 3)^2 = 25
```

### Объяснение кода
1. **Пример 1**:
    - `3 + 4 * 5`: Сначала выполняется умножение `4 * 5`, затем сложение с `3`.
    - Результат: `23`.

2. **Пример 2**:
    - `(3 + 4) * 5`: Скобки заставляют сначала сложить `3` и `4`, а затем умножить результат на `5`.
    - Результат: `35`.

3. **Пример 3**:
    - `20 / 4 % 3`: Сначала деление `20 / 4`, затем вычисляется остаток от деления результата на `3`.
    - Результат: `2`.

4. **Пример 4**:
    - `Math.Pow(2, 3) + 5`: Сначала выполняется возведение `2` в степень `3`, затем результат складывается с `5`.
    - Результат: `13.0`.

5. **Пример 5**:
    - `Math.Pow(2 + 3, 2)`: Скобки изменяют порядок операций, и сначала вычисляется сумма `2 + 3`, затем результат возводится в квадрат.
    - Результат: `25.0`.

### Вывод
Использование скобок позволяет менять порядок выполнения операций, чтобы добиться нужного результата. 
Если не указывать скобки, C# применяет стандартные правила приоритета операций.

## Переменные булева типа
...
Тип `char` в C# представляет собой 16-битное целое значение, представляющее символ Unicode. Многие операции, применимые 
к целочисленным типам, также могут быть использованы с `char`, поскольку он имеет числовую природу. Однако некоторые операции, 
такие как логические (например, `<`, `>`, `<=`, `>=`), имеют ограниченное применение для `char`, поскольку их результатом 
является `bool`, а `char` предназначен для работы с символами, а не булевыми значениями.

#### 1. Арифметические операции {id="1_5"}
Можно выполнять следующие арифметические операции с символами, поскольку их значения основаны на числовом представлении Unicode:
- `+` (Сложение)
- `-` (Вычитание)
- `*` (Умножение)
- `/` (Деление)
...
```c#
char a = 'A';  // Unicode 65
char b = (char)(a + 1);  // 'B' (Unicode 66)
char c = (char)(b - 1);  // 'A' (Unicode 65)
int product = a * 2;     // 130
```

#### 2. Инкремент и декремент {id="2_5"}
Операции инкремента (`++`) и декремента (`--`) можно применять:
- Префиксный и постфиксный инкремент увеличивает значение Unicode на 1.
- Префиксный и постфиксный декремент уменьшает значение Unicode на 1.
...
#### 3. Побитовые операции {id="3_5"}
Поскольку `char` является 16-битным числом, к нему применимы побитовые операции:
- `&` (Побитовая И)
- `|` (Побитовая ИЛИ)
- `^` (Побитовая исключающая ИЛИ)
- `~` (Побитовое НЕ)
...
```c#
char e = 'A';  // 65
char f = (char)(e & 0xFF);  // Побитовая И: 65 ('A')
char g = (char)(e | 0x20);  // Побитовая ИЛИ: 97 ('a')
```

#### 4. Логические операции {id="4_2"}
Логические операции доступны, но результатом всегда будет `bool`:
- `==` (Равно)
- `!=` (Не равно)
- `<`, `>`, `<=`, `>=` (Сравнение на основе Unicode)

```c#
char h = 'A';
char i = 'B';
bool isEqual = h == i;  // false
bool isLess = h < i;    // true
```

#### 5. Операции присваивания {id="5_3"}
Все операции присваивания, доступные для `int`, применимы и к `char`:
...
char j = 'A';
j += 1;  // 'B'
j &= (char)0xFF;  // Остается 'B'
```

#### 6. Унарные операции {id="6_2"}
- `+` (Явное указание положительности, редко используется)
- `-` (Смена знака невозможна, поскольку тип `char` беззнаковый)

```c#
char k = 'A';
int unicodeValue = +k;  // 65
```

#### 7. Приведение типов {id="7_2"}
Тип `char` может быть преобразован в другие числовые типы:
- Неявное приведение: `char` → `int`
- Явное приведение: `int` → `char`
...
#### 8. Операции проверки переполнения {id="8_1"}
Операции проверки переполнения (`checked` и `unchecked`) работают с `char`, как и с числовыми типами:

```c#
char n = (char)65535;  // Максимальное значение char
char o = unchecked((char)(n + 1));  // Переполнение: 0
```

Практика!

Создай консольное приложение `ex0030_char_operations` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
Инкремент: B
Декремент: A
Побитовая И: A & 0xFF = A     
Побитовая ИЛИ: A | 0x20 = a   
Равно: A == B = False
Меньше: A < B = True
Сложение с присваиванием: B   
Побитовая И с присваиванием: B
Унарная операция +: 65        
Неявное приведение: 65        
Явное приведение: B
Переполнение:  (Unicode 0)
```

### Итог:
- **Все основные операции для `int` применимы к `char`, за исключением смены знака.**
- `char` можно считать беззнаковым 16-битным целым типом, с некоторыми особенностями, связанными с его природой символа.

## Переменные типа перечисления

Перечисление (тип `enum`) в C# — это способ создать свой тип данных, который позволяет задавать фиксированный набор 
именованных значений. Эти значения представляют числа, но вместо работы с числами мы используем понятные имена. 
Это упрощает написание и чтение кода.

### Основные свойства перечислений:

1. **Синтаксис создания**:
   ```c#
   enum <Имя> : <Базовый_тип>
   {
   ...
   ```
    - `<Имя>` — это название вашего нового типа данных.
    - `<Базовый_тип>` (необязательный) — тип данных, который будет использоваться для хранения значений (по умолчанию это `int`).
    - `<Идентификатор>` — это название каждого элемента перечисления. Идентификатором может быть любое валидное имя, начинающееся с буквы, например, `Monday`, `Error`, `Ready`.
    - `<значение>` — числовое значение, связанное с элементом. Если значение не указано, оно присваивается автоматически, начиная с 0.
...
enum DaysOfWeek
{
    Sunday = 0,  // Явно указано значение
    Monday,      // Присвоится 1
    Tuesday,     // Присвоится 2
    Wednesday,   // Присвоится 3
    Thursday,    // Присвоится 4
    Friday,      // Присвоится 5
    Saturday     // Присвоится 6
}
```

### Использование перечисления:

Переменные типа перечисления можно создавать так же, как переменные стандартных типов, например `int`.

```c#
DaysOfWeek today = DaysOfWeek.Wednesday;
```

### Можно ли использовать числа напрямую?

Да, можно, но с явным приведением типа. Например:
```c#
DaysOfWeek today = (DaysOfWeek)1; // Присвоится Monday
Console.WriteLine(today);        // Выведет "Monday"
```

Если попытаться присвоить число без приведения, это вызовет ошибку компиляции:
```c#
// Ошибка:
DaysOfWeek today = 1;
```

За мечь, падаван! Вернее за клаву!

Создай консольное приложение `ex0030_enum_var` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
Для вывода всех элементов перечисления используется цикл `foreach` который перебирает все элементы перечисления.
Циклы мы будем проходить чуть позже.

Вывод программы:

```
Сегодня: Wednesday, Код: 3
Завтра: Thursday      
Все дни недели:       
Sunday = 0
Monday = 1
...
Friday = 5
Saturday = 6
День с кодом 1: Monday
```

### Особенности перечислений:

1. **Идентификатор**:
    - Это просто имя, которое вы даете каждому элементу перечисления, чтобы обращаться к ним по смыслу, а не по числам.
    - Пример: `Monday`, `Tuesday` — это идентификаторы, которые представляют числа `1` и `2`.

2. **Числовое значение**:
    - Каждый элемент перечисления связан с числовым значением.
    - Если значение не указано, оно назначается автоматически, начиная с 0.

3. **Базовый тип**:
    - По умолчанию это `int`, но можно использовать любой целочисленный тип:
      ```c#
...
4. **Приведение типов**:
    - Числа можно преобразовать в значения перечисления и наоборот, но это нужно делать явно:
      ```c#
      DaysOfWeek day = (DaysOfWeek)5;  // Friday
      int dayCode = (int)DaysOfWeek.Sunday;  // 0
      ```

5. **Методы для работы с перечислениями**:
    - `Enum.GetValues()` — возвращает все элементы перечисления.
    - `Enum.GetName()` — возвращает имя элемента по числовому значению.

### Почему использовать перечисления?

1. **Улучшение читаемости**:
    - Вместо `int day = 3` вы пишете `DaysOfWeek day = DaysOfWeek.Wednesday`, что намного понятнее.

2. **Защита от ошибок**:
    - Переменная перечисления может принимать только заданные значения, что предотвращает случайные ошибки.

3. **Простота работы с фиксированными наборами значений**:
    - Например, вместо проверки `if (day == 1)` лучше писать `if (day == DaysOfWeek.Monday)`.
 
### Итог:

Перечисления позволяют задавать ограниченный набор именованных констант, которые можно использовать в коде как новые типы. 
Это делает код проще и понятнее. Несмотря на связь с числами, они обеспечивают строгую типизацию и исключают случайные ошибки.

## Операции над типом перечислений

В C# перечисления (тип `enum`) поддерживают различные операции, которые позволяют работать с ними эффективно. 
Вот список всех доступных операций с примерами.

### 1. Присваивание {id="1_2"}

Можно присвоить значение из перечисления или привести число к перечислению.

#### Пример:
```c#
enum DaysOfWeek { Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday }

DaysOfWeek today = DaysOfWeek.Wednesday;  // Присваивание из перечисления
DaysOfWeek tomorrow = (DaysOfWeek)4;     // Присваивание с приведением числа
```

### 2. Приведение типов {id="2_2"}

Можно преобразовывать значения перечисления в числовые типы и наоборот.

#### Пример:
```c#
int code = (int)DaysOfWeek.Friday;  // Преобразование в число: 5
DaysOfWeek day = (DaysOfWeek)2;    // Преобразование числа в перечисление: Tuesday
```

### 3. Сравнение {id="3_3"}

Перечисления поддерживают сравнение с использованием операторов `==`, `!=`, `<`, `>`, `<=`, `>=`.
...
if (today > DaysOfWeek.Monday)
    Console.WriteLine("Сегодня позже понедельника.");
```

### 4. Побитовые операции {id="4_3"}

Перечисления можно использовать для выполнения побитовых операций, если они помечены атрибутом `[Flags]`.
...
C# предоставляет встроенные методы для работы с перечислениями.

- **`Enum.GetName()`** — получает имя элемента по значению.
- **`Enum.GetValues()`** — возвращает массив всех значений.
- **`Enum.GetNames()`** — возвращает массив имен.
- **`Enum.IsDefined()`** — проверяет, существует ли элемент с указанным значением.
- **`Enum.Parse()`** — преобразует строку в элемент перечисления.
- **`Enum.TryParse()`** — преобразует строку в элемент перечисления с безопасностью.
...
bool isValid = Enum.IsDefined(typeof(DaysOfWeek), 2); // True
...
### 6. Операции с использованием строк {id="6_3"}

Можно преобразовывать перечисления в строки и обратно.
...
Console.WriteLine(parsedDay);               // Monday
```

### 7. Установка значений по умолчанию {id="7_3"}

Если переменная типа `enum` не инициализирована, она принимает значение `0`. Это значение должно соответствовать одному 
из элементов перечисления, или использовать специальный элемент, например, `None`.

#### Пример:
```c#
enum Status { None, Success, Error }

Status currentStatus = default;
Console.WriteLine(currentStatus); // Output: None (значение 0)
```

### 8. Операции с флагами {id="8_2"}

Если перечисление помечено атрибутом `[Flags]`, с ним можно выполнять побитовые операции: объединение (`|`), 
проверка наличия (`&`), удаление (`~`).

#### Пример:
```c#
[Flags]
enum AccessRights { None = 0, Read = 1, Write = 2, Execute = 4 }
...
// Проверка права
if ((rights & AccessRights.Execute) != 0)
    Console.WriteLine("Есть право на выполнение.");

// Удаление права
rights &= ~AccessRights.Write;
Console.WriteLine(rights); // Output: Read, Execute
```

### 9. Перебор значений

С помощью `Enum.GetValues()` можно перебирать элементы перечисления, например, для вывода списка.
...
### 10. Определение следующего/предыдущего элемента

С помощью приведения к числам можно определять следующий или предыдущий элемент.

#### Пример:
```c#
DaysOfWeek today = DaysOfWeek.Tuesday;
DaysOfWeek tomorrow = (DaysOfWeek)((int)today + 1); // Wednesday
Console.WriteLine($"Сегодня: {today}, Завтра: {tomorrow}");
```

А теперь соберем все это в одной программе и попрактикуемся.

Создай консольное приложение `ex0031_enum_operations` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
```
Сегодня: Wednesday
Код сегодняшнего дня: 3   
Завтра: Thursday
Завтра позже, чем сегодня.
...
Перечисления (`enum`) в C# — это мощный инструмент для создания именованных констант, который поддерживает множество 
операций, включая преобразования, сравнения, переборы и использование побитовых операций.

## Переменные типа структура

Структура (`struct`) в C# — это тип данных, который позволяет объединять несколько полей (переменных) разных типов в 
один логический блок. Она похожа на класс (`class`), но имеет свои особенности. Основное отличие заключается в том, 
что структуры являются **типами значений**, а классы — **ссылочными типами**.

### Почему `struct` — это тип значения, а не ссылочный тип? {id="struct_1"}

#### Разница между типами значений и ссылочными типами:

1. **Типы значений (например, `int`, `double`, `struct`)**:
    - Хранят данные **прямо внутри своей переменной**.
    - При копировании переменной создаётся **полная копия данных**.
    - Не используют кучу (heap), если не находятся внутри объекта класса.

2. **Ссылочные типы (например, `class`, массивы, `string`)**:
    - Хранят **ссылку на область памяти**, где находятся данные.
    - При копировании переменной копируется **только ссылка**, а данные остаются общими.

### Особенности `struct`:

1. **Где в памяти создаются экземпляры `struct`?**
    - Если структура объявлена **вне класса и как локальная переменная** (например, внутри метода), она создаётся в **стеке**.
        - **Стек** — это область памяти, где данные живут, пока метод выполняется.
    - Если структура является **полем класса**, она хранится в **куче**, но данные структуры остаются независимыми.
        - **Куча (heap)** — это область памяти для объектов, которые живут дольше времени выполнения метода.

2. **Копирование по значению:**
    - Когда вы присваиваете одну переменную типа `struct` другой, копируются **данные**, а не ссылка.

3. **Меньше затрат на память и быстрее:**
    - Из-за хранения в стеке структура легче по весу и быстрее в использовании.

4. **Нет наследования:**
    - Структуры не могут наследоваться от других структур или классов, но могут реализовывать интерфейсы.

### Сравнение структуры и класса

| **Характеристика**            | **`struct`**                          | **`class`**                         |
|-------------------------------|----------------------------------------|--------------------------------------|
| **Тип**                      | Значение                              | Ссылка                              |
| **Хранение**                 | Стек (или внутри другого значения)    | Куча (heap)                         |
| **Передача**                 | Копируется полностью                  | Передаётся ссылка                   |
| **Наследование**             | Не поддерживает                       | Поддерживает                        |
| **Конструкторы**             | Может содержать                       | Обязательно содержит                |
| **Неявное значение по умолчанию** | Все поля инициализируются значениями по умолчанию | Ссылка `null`                   |

Попрактикуемся в создании обычной (изменяемой) структуры.

Создай консольное приложение `ex0032_struct_var` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.

Приведите `Program.cs` к следующему виду:

#### **Код с изменяемой структурой:**
...
**Важно:** В данном примере структура `Rectangle` создаётся в **стеке**, так как она является локальной переменной 
внутри метода `Main`.

>Что такое создание экземпляра структуры, что значит конструктор структуры, а так же вызов её методов мы узнаем когда 
> будем изучать классы. Пока мы просто знакомимся с этим типом 😊.
...
Площадь: 32,85
Новая ширина: 10
Площадь после изменения: 73
```

Если вы хотите сделать структуру неизменяемой, можно использовать `readonly` для полей.

Попрактикуемся в создании структуры с неизменяемыми полями.

Создай консольное приложение `ex0033_struct_read_only_var` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
```
Площадь: 32,85
Старая ширина: 4,5, новая ширина: 10
Площадь нового прямоугольника: 73
```

Если сможете, постарайтесь проанализировать и понять почему эти две программы выводят такие значения. Если не получится 
не страшно. Когда начнем изучать классы всё поймете.

### **Дополнительные пояснения:**

1. **Идентификатор структуры:**
    - В примере `Rectangle` — это имя типа, как `int` или `double`. Его можно использовать для создания переменных, например:
      ```c#
      Rectangle myRect = new Rectangle(3, 6);
      ```

2. **Почему структуры копируются?**
    - Когда вы создаёте новую переменную на основе структуры, все её данные копируются:
      ```c#
      Rectangle rect1 = new Rectangle(4, 5);
      Rectangle rect2 = rect1; // Копия структуры
      rect2.Width = 10;
      // rect1.Width останется 4, так как копируется по значению.
      ```
...
## Переменные типа кортежей

**Кортежи** — это удобный способ объединить несколько значений разных типов в одну переменную без необходимости 
создавать отдельный класс или структуру. Они особенно полезны, когда нужно быстро вернуть несколько значений из 
метода или передать связанные данные в одном объекте.

>В C# нет отдельного ключевого слова, которое прямо указывает на то, что переменная является кортежем. 
>Вместо этого кортежи создаются с помощью специального синтаксиса (круглых скобок), а их типы описываются 
> с использованием встроенной поддержки кортежей в .NET.
{style="note"}

### Почему нет ключевого слова?
Тип кортежа — это структура, определённая в пространстве имён `System`. Например:
```c#
System.ValueTuple<int, string> myTuple = (42, "Hello");
```
Таким образом, кортежи представляют собой типы значений (`struct`), а синтаксис с круглыми скобками `(T1, T2)` — это просто 
удобный способ работы с `ValueTuple`.

Эта информация была дана для справки. Для пущего понимания 😊

### Кортежи в C#: структурные и классовые

1. **Современные кортежи** (`ValueTuple`)
    - Современные кортежи, появившиеся с C# 7.0, представляют собой **структуры**.
    - Они легковесны и позволяют создавать как **именованные**, так и **неименованные** кортежи.
    - Например:
      ```c#
      (int age, string name) person = (25, "Alice");
      ```
    - Предпочтительны для использования в современном коде.

2. **Класс Tuple**
    - Старый способ работы с кортежами через класс **`Tuple`**. Упомянут здесь для информации.
    - Объекты класса `Tuple` неизменяемы (**immutable**) и их элементы нельзя менять после создания.
    - Например:
      ```c#
      var oldTuple = Tuple.Create(1, "Alice");
      ```

### Разница между `Tuple` и `ValueTuple`
| **Свойство**            | **`Tuple` (старый)**                     | **`ValueTuple` (новый)**               |
|--------------------------|------------------------------------------|----------------------------------------|
| **Тип**                 | Класс (ссылочный тип, `class`)           | Структура (тип значения, `struct`)     |
| **Память**              | Данные хранятся в куче                  | Данные хранятся в стеке (если локально) |
| **Производительность**  | Более медленный из-за выделения памяти в куче | Быстрее благодаря хранению в стеке     |
| **Читаемость кода**     | Поля именуются `Item1`, `Item2`, ...     | Поддержка именованных элементов        |
| **Изменяемость**        | Неизменяемый (immutable)                 | Изменяемый (mutable)                   |
| **Поддержка языка**     | Требуется явный вызов `Tuple.Create`      | Поддержка синтаксиса кортежей (`(a, b)`) |
| **Введён в версии**     | .NET Framework 4.0                      | C# 7.0                                 |

### Особенности современных кортежей

- **Кортежи — это структура**:  
  Современные кортежи реализованы как структуры (`ValueTuple`), что означает, что они являются **типами значений** и создаются в стеке (если это локальная переменная).

- **Именованные и неименованные элементы**:  
  Вы можете дать элементам кортежа имена для более удобного использования.

- **Поддержка возврата нескольких значений из методов**:  
  Кортежи позволяют легко возвращать сразу несколько значений:
  ```c#
  (int, string) GetPerson() => (25, "Alice");
  ```

- **Совместимость**:  
  Класс `Tuple` существует и в современных версиях C#, но его использование не рекомендуется, если доступен `ValueTuple`.

### Примеры создания кортежей

1. **Неименованный кортеж** (обращение к элементам кортежа по **позиции** элемента):
   ```c#
   var person = (25, "Alice");
...
2. **Именованный кортеж** (обращение к элементам кортежа по **имени** элемента):
   ```c#
   (int age, string name) person = (25, "Alice");
...
3. **Объект класса Tuple** (*устаревшее*):
   ```c#
   var personTuple = Tuple.Create(25, "Alice");
   Console.WriteLine(personTuple.Item1); // 25
   Console.WriteLine(personTuple.Item2); // Alice
   ```

Попрактикуемся с кортежами.

Создай консольное приложение `ex0034_tuple_var` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
Boolean: True, Double: 3,14, String: Hello
Age: 25, Name: Alice
```

### Построчное объяснение программы

1. **Неименованный кортеж**:
   ```c#
   var point = (10, 20);
   Console.WriteLine($"X: {point.Item1}, Y: {point.Item2}");
   ```
   Здесь создаётся кортеж с двумя элементами, доступ к которым осуществляется через `Item1` и `Item2`.

2. **Именованный кортеж**:
   ```c#
   (int x, int y) namedPoint = (30, 40);
   WriteLine($"X: {namedPoint.x}, Y: {namedPoint.y}");
   ```
   Элементы кортежа имеют имена (`x` и `y`), что делает код более читаемым.
...
3. **Возврат значений из метода**:
   ...
   ```
   Метод возвращает кортеж с тремя именованными элементами.

4. **Смешанные типы в кортеже**:
   ```c#
   var mixed = (true, 3.14, "Hello");
   Console.WriteLine($"Boolean: {mixed.Item1}, Double: {mixed.Item2}, String: {mixed.Item3}");
   ```
   Кортеж может содержать значения разных типов.

5. **Класс `Tuple`**:
   ```c#
   var oldTuple = Tuple.Create(25, "Alice");
   Console.WriteLine($"Age: {oldTuple.Item1}, Name: {oldTuple.Item2}");
   ```
   Объект класса `Tuple` создаётся с помощью метода `Create`. Он неизменяем, а доступ к элементам осуществляется через свойства `Item1`, `Item2` и т. д.

### Резюме

- Используйте современные кортежи (`ValueTuple`) для работы с данными. Они легче и удобнее.
- `Tuple` — устаревший способ, но всё ещё может использоваться в старых приложениях.
- Кортежи полезны для временного хранения нескольких связанных данных без создания полноценного класса или структуры.


## Операции над кортежами

1. **Создание кортежей**
    - С использованием `ValueTuple` или синтаксиса `(тип1, тип2, ...)`.

2. **Доступ к элементам кортежа**
    - По именам или через `.Item1`, `.Item2` и так далее.

3. **Декомпозиция кортежей**
    - Присваивание значений кортежа отдельным переменным.

4. **Присваивание кортежей**
    - Кортежи можно напрямую присваивать друг другу.

5. **Сравнение кортежей**
    - Кортежи поддерживают операторы сравнения `==` и `!=`.

6. **Вложенные кортежи**
    - Кортежи могут содержать другие кортежи.

7. **Возврат кортежа из метода**
    - Кортежи часто используются для возврата нескольких значений из метода.

8. **Изменение значений кортежа**
    - Только если кортеж объявлен как `ref`.

9. **Работа с кортежами в коллекциях**
    - Кортежи можно хранить в массивах, списках и других коллекциях.

10. **Сопоставление с образцом (Pattern Matching)**


И опять практика, падаван.

Создай консольное приложение `ex0035_tuple_operations` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
1. **Создание кортежей**:
    - `tuple1` — неименованный кортеж.
    - `tuple2` — именованный кортеж, где элементы имеют названия `Id`, `Name`, `IsActive`.

2. **Доступ к элементам**:
    - Через `.Item1`, `.Item2` (неименованный кортеж).
    - Через имена, например, `Name` (именованный кортеж).

3. **Декомпозиция**:
    - Значения из кортежа `tuple2` распаковываются в отдельные переменные `id`, `name`, `isActive`.

4. **Присваивание**:
    - Кортеж `tuple3` изменяется через новое присваивание.

5. **Сравнение**:
    - Кортежи `t1` и `t2` сравниваются на равенство.

6. **Вложенные кортежи**:
    - Кортеж `nestedTuple` содержит другой кортеж как элемент.

7. **Возврат кортежа из метода**:
    - Метод `GetUserInfo` возвращает кортеж с информацией о пользователе.

8. **Изменение значений через `ref`**:
    - Метод `ModifyTuple` изменяет значения кортежа, переданного по ссылке.

9. **Кортежи в коллекциях**:
    - Используем массив кортежей `(int, string)` и проходим по нему с помощью `foreach`.

10. **Сопоставление с образцом**:
    - Проверяем, совпадает ли `tuple2` с заданным шаблоном `(42, _, false)`.

### Вывод программы

```
Кортеж 1: (1, Hello, True)
Кортеж 2: (42, Alice, False)
Первый элемент кортежа 1: 1
Имя из кортежа 2: Alice
Декомпозиция: Id = 42, Name = Alice, IsActive = False
Новый кортеж: (20, C#)
Равны ли т1 и т2? True
Вложенный кортеж: A, 2,5
Пользователь: Id = 101, Name = John Doe, Role = Admin
Модифицированный кортеж: X = 20, Y = 30
Коллекция кортежей: Id = 1, Name = Item1
Коллекция кортежей: Id = 2, Name = Item2
Коллекция кортежей: Id = 3, Name = Item3
Совпало с шаблоном (42, _, false)
```

Этот пример охватывает практически все возможные операции с кортежами в C#.

## Типы значений допускающие значение NULL

Nullable value types (nullable-типы) в C# — это специальный вид типов, которые могут хранить не только значения, но и значение `null`. Обычно `null` означает, что значение отсутствует или неизвестно.

Обычно, простые типы данных (например, `int`, `double`, `bool`) не могут быть `null`. Например, если у нас есть переменная типа `int`, то она всегда содержит какое-то число, например `0`, `1`, `-5` и так далее.

Но бывают ситуации, когда нужно сказать: *"Здесь пока нет значения"*. Например:
- Когда мы запрашиваем информацию у пользователя, но он ещё ничего не ввёл.
- Когда значение временно недоступно или отсутствует по какой-то причине.
- Информация запрашивается из базы данных, но там не заполнено значение.

Вот тут и приходят на помощь nullable-типы.

### Как создать nullable value type?

Чтобы сделать обычный тип "nullable", достаточно добавить знак `?` после типа. Например:
```c#
int? number = null; // Это nullable int, может быть либо числом, либо null
bool? isAvailable = null; // Это nullable bool, может быть true, false или null
```

### Как использовать nullable value types?

1. **Присвоение значения `null`**:  
   Мы можем присвоить переменной значение `null`, чтобы указать, что сейчас значения нет.
   ```c#
   int? age = null; // Возраст неизвестен
   ```

2. **Проверка на наличие значения**:  
   Чтобы узнать, есть ли у nullable-типа значение, используют свойство `.HasValue` или проверку на `null`:
   ```c#
   if (age.HasValue)
   {
...
3. **Доступ к значению**:  
   Если значение есть, его можно получить через свойство `.Value`:
   ```c#
   int? age = 25;
   Console.WriteLine(age.Value); // Выведет 25
   ```

### Ещё раз на пальцах объясняю для закрепления
Представь, что ты ведёшь таблицу с результатами проверки навыков падаванов. У тебя есть список падаванов, и ты хочешь 
записать, прошёл ли падаван проверку:
- Если падаван прошёл проверку, ставишь `true`.
- Если нет, ставишь `false`.
- Если падаван вообще не пришёл, оставляешь пусто.

В обычной ситуации ты не сможешь оставить "пусто", потому что `bool` всегда должен быть либо `true`, либо `false`.

С nullable-типа это становится возможным:
```c#
bool? passedExam = null; // Ученик не пришёл на контрольную
```

### Откуда берётся значение `null` для `bool`?
Значение `null` может появляться в nullable-типах, если:
1. Переменной изначально не присвоено значение:
   ```c#
   bool? isOnline = null; // Пользователь ещё не зашёл в чат
   ```
2. Значение берётся из базы данных или другого источника, где может быть "пусто" (например, из формы ввода, если пользователь ничего не выбрал).

### Зачем это нужно?
Nullable value types нужны, чтобы:
1. **Указать, что значение может отсутствовать.**
2. **Избежать ошибок.** Без nullable-типов нам пришлось бы придумывать специальные значения, чтобы сказать "значения нет" (например, для возраста мы могли бы писать `-1`, но это неудобно и неочевидно).
3. **Работать с базами данных**, где поля могут быть пустыми (NULL).

### Простой пример
```c#
int? temperature = null; // Температура неизвестна
temperature = 20; // Позже мы узнаём, что температура 20
...
**Итог:** nullable value types — это способ хранить дополнительные состояния (например, "значения нет") для обычных типов данных.

Попрактикуемся, падаван.

Создай консольное приложение `ex0036_null_var` при помощи шаблона `tinyconsole` в папке
`episode02` и добавь его в файл решения `episode02.sln`. Это можно сделать как в командной строке, так и в любой IDE.
...
```
Пользователь онлайн? Неизвестно
